...
class PenugasanController extends Controller
{
    ...
    {
        $penugasan = Penugasan::with(['jadwal.jenisPekerjaan', 'anggota'])
            ->when($request->filled('q'), fn ($query) => $query->whereHas('jadwal', fn ($schedule) => $schedule->where('nama_kegiatan', 'like', '%'.$request->q.'%'))->orWhereHas('anggota', fn ($member) => $member->where('nama_lengkap', 'like', '%'.$request->q.'%')))
    ...
    {
        $jadwal = \App\Models\Jadwal::with('jenisPekerjaan')->whereIn('status', ['draft', 'scheduled'])->orderBy('tanggal')->get();
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $schedule = \App\Models\Jadwal::with('jenisPekerjaan')->findOrFail($validated['jadwal_id']);
        $member = User::where('role', 'anggota')->where('status_aktif', true)->findOrFail($validated['anggota_id']);

        if (! $member->kompetensi()->where('jenis_pekerjaan_id', $schedule->jenis_pekerjaan_id)->exists()) {
            return back()->withInput()->withErrors(['anggota_id' => 'Anggota tidak memiliki kompetensi yang sesuai.']);
        }
